...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        q_value = self.values[(state,action)] # Caculates the q value of a given action in a aiven state
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(possible_actions) == 0: # If there is no legal actions, return 0 - terminal state
          ...
          for action in possible_actions:
            if self.getQValue(state, action) == max_q: # If current action q value is equal to the max q value - its the best action
    ...
    def getAction(self, state):
        ...
        if util.flipCoin(self.epsilon) == True: # Flipping coin which will be true for epsilon amount of time
          action = random.choice(possible_actions) # Choose random action epsilon amount of times - explor new actions to cover search space
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.values[(state,action)] = current_state + self.alpha*(reward + self.discount*(next_state) - current_state) #Bellman equation
